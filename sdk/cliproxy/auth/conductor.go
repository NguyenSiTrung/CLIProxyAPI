...
func (m *Manager) pickNextMixed(ctx context.Context, providers []string, model string, opts cliproxyexecutor.Options, tried map[string]struct{}) (*Auth, ProviderExecutor, string, error) {
	...
	// Fall back to legacy path when access key auth bindings are configured,
	// because the scheduler fast path does not filter by allowlist.
	if accessKey := accessKeyFromOptions(opts); accessKey != "" {
		if cfg, _ := m.runtimeConfig.Load().(*internalconfig.Config); cfg != nil && len(cfg.AccessKeyAuths) > 0 {
			return m.pickNextMixedLegacy(ctx, providers, model, opts, tried)
		}
	}
